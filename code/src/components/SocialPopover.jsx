    ...
    return (
        <div className="relative inline-block">
            <button
                ...
                className="flex items-center space-x-2 rounded-lg bg-white/5 px-4 py-2 text-sm backdrop-blur-lg transition-all hover:bg-white/10"
            >
            ...
            {isOpen && (
                <div
                    ...
                    onMouseLeave={() => setIsOpen(false)}
                >
                    <div className="w-64 rounded-xl border border-white/10 bg-[#1a1b26]/95 p-3 backdrop-blur-xl shadow-lg">
                            ...
                            {socialLinks.map((social) => (
                                <div key={social.name} className="group relative flex items-center justify-between rounded-lg px-2 py-1.5 transition-colors hover:bg-white/10">
                                    ...
                                    >
                                        ...
                                        {/* <p className="text-xs text-white/70">@{truncateText(social.username, 12)}</p> */}
                                    </a>
                                    <div className="flex opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            onClick={() => copyToClipboard(social.url)}
                                            className="rounded-md p-1 text-white/70 hover:bg-white/20 transition-colors"
                                            title="Copy URL"
                                        >
                    ...
                    {copiedText && (
                        <div className="fixed top-2 right-6 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
                            <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-[#1a1b26]/95 px-3 py-2 shadow-lg backdrop-blur-xl">
                                <Check className="h-3.5 w-3.5 text-emerald-500" />
                                <span className="text-xs text-white/90">Copied to clipboard</span>
                            </div>
                        </div>
                    )}
                </div>

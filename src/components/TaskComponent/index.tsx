...
const TaskComponent = () => {
  ...
  const handleAddTask = (values: {
    ...
    if (values.title.trim()) {
      ...
      const updatedFolders = calculateFolderCounts(folders, [
        ...tasks,
        newTask,
      ]);
      dispatch(setFolders(updatedFolders));
  ...
  const handleEditTask = (taskId: string, updates: Partial<Task>) => {
    ...
    const updatedTasks = tasks.map((t) =>
      t.id === taskId ? { ...t, ...updates, lastModified: Date.now() } : t,
    );
    const updatedFolders = calculateFolderCounts(folders, updatedTasks);
    dispatch(setFolders(updatedFolders));
    ...
    <div className="h-full flex">
      ...
      >
        ...
        <div className="space-y-1">
          {folders
            .slice()
            .sort((a, b) =>
              a.id === 'archive' ? 1 : b.id === 'archive' ? -1 : 0,
            )
            .map((folder) => (
              <motion.div
                key={folder.id}
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold tq-text-primary">
            {headerTitle}
          </h1>
          ...
          <AnimatePresence mode="popLayout">
            ...
            {filteredTasks.map((task) => (
              <TaskItem
                ...
                onDelete={() =>
                  setShowDeleteConfirm({ type: 'task', id: task.id })
                }
              />
            ))}
          </AnimatePresence>

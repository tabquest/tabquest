...
const BookmarkComponent = () => {
  ...
  const { folders, bookmarks } = useAppSelector((state) => state.bookmarks);
  ...
  const handleUpdateBookmark = (
    id: string,
    updates: { title: string; url: string; tags: string[] },
  ) => {
  ...
  const handleAddBookmark = (bookmark: Record<string, string>) => {
    if (bookmark.title.trim() && bookmark.url.trim()) {
      ...
      const newBookmark: Bookmark = {
        ...
        dateAdded: Date.now(),
        starred: false,
        originalFolder: selectedFolder || undefined,
      };
  ...
  const handleStarBookmark = (bookmark: Bookmark) => {
